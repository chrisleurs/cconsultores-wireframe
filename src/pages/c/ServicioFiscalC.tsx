...
import { getFiscalContent } from "@/i18n/fiscalContent";
import { t } from "@/i18n/ui";

export default function ServicioFiscalC() {
  const [openFaq, setOpenFaq] = useState<number | null>(null);
  const lang = useLang();
  const c = getFiscalContent(lang);
  const base = lang === "en" ? "/en/version-c" : "/version-c";

  return (
    <div className="min-h-screen">
      <SEO title={c.seo.title} description={c.seo.description} path={`${base}/servicios/fiscal`} />
      <ServiceJsonLd name={c.jsonLd.name} description={c.jsonLd.description} url={`https://cconsultores-wireframe.lovable.app${base}/servicios/fiscal`} serviceType="TaxAdvisory" />
      <FaqJsonLd faqs={c.faqs} />
      <Navbar version="c" />

      <section className="min-h-[70dvh] relative flex items-center justify-center text-center">
        <img src="https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=1600&h=900&fit=crop" alt={c.heroH1} className="absolute inset-0 w-full h-full object-cover" />
      ...
      <section className="bg-white py-24 px-5 md:px-10">
        <div className="max-w-[900px] mx-auto">
          <p className="label-uppercase text-camhaji-muted mb-4">{c.problemLabel}</p>
          <h2 className="font-sans font-bold text-camhaji-text mb-8" style={{ fontSize: "clamp(24px, 3.5vw, 40px)", letterSpacing: "-0.03em" }}>{c.problemTitle}</h2>
          <p className="font-sans text-[16px] font-light text-camhaji-muted leading-relaxed mb-6">{c.problemP1}</p>
          <p className="font-sans text-[16px] font-light text-camhaji-muted leading-relaxed mb-4">{c.problemP2}</p>
          <p className="font-sans text-[16px] font-light text-camhaji-muted leading-relaxed">{c.problemP3}</p>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-0">
        ...
        <div className="order-1 md:order-2"><img src="https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=1200&h=800&fit=crop" alt={c.splitTitle} className="w-full h-full object-cover aspect-[4/3]" /></div>
      </section>

      <section className="bg-surface py-24 px-5 md:px-10">
        <div className="max-w-[900px] mx-auto">
          <p className="label-uppercase text-camhaji-muted mb-4">{c.enfoqueLabel}</p>
          <h2 className="font-sans font-bold text-camhaji-text mb-8" style={{ fontSize: "clamp(22px, 3vw, 36px)", letterSpacing: "-0.02em" }}>{c.enfoqueTitle}</h2>
          <p className="font-sans text-[16px] font-light text-camhaji-muted leading-relaxed mb-6">{c.enfoqueP1}</p>
          <p className="font-sans text-[16px] font-light text-camhaji-muted leading-relaxed">{c.enfoqueP2}</p>
        </div>
      </section>

      <section className="bg-camhaji-base py-24 px-5 md:px-10">
        <div className="max-w-[900px] mx-auto">
          <p className="label-uppercase text-white/30 mb-4">{c.servLabel}</p>
          <h2 className="font-sans font-bold text-white mb-16" style={{ fontSize: "clamp(24px, 3vw, 36px)", letterSpacing: "-0.02em" }}>{c.servTitle}</h2>
      ...
      <section className="bg-white py-24 px-5 md:px-10">
        <div className="max-w-[900px] mx-auto">
          <p className="label-uppercase text-camhaji-muted mb-4">{c.porqueLabel}</p>
          <h2 className="font-sans font-bold text-camhaji-text mb-8" style={{ fontSize: "clamp(22px, 3vw, 36px)", letterSpacing: "-0.02em" }}>{c.porqueTitle}</h2>
          <p className="font-sans text-[16px] font-light text-camhaji-muted leading-relaxed mb-4">{c.porqueP1}</p>
          <p className="font-sans text-[16px] font-light text-camhaji-muted leading-relaxed mb-4">{c.porqueP2}</p>
          <p className="font-sans text-[16px] font-light text-camhaji-muted leading-relaxed">{c.porqueP3}</p>
        </div>
      </section>

      <section className="min-h-[40vh] relative flex items-center justify-center text-center">
        <img src="https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=1600&h=800&fit=crop" alt={c.bandText} className="absolute inset-0 w-full h-full object-cover" />
      ...
      <section className="bg-camhaji-base py-20 px-5 md:px-10">
        <div className="max-w-[820px] mx-auto">
          ...
          <blockquote className="font-serif italic text-white/80 mb-8" style={{ fontSize: "clamp(18px, 2.4vw, 28px)", lineHeight: 1.5 }}>"{testimonials.david.short}"</blockquote>
      ...
      <ServiceExtras article={blogByService.fiscal} related={relatedByService.fiscal} serviceLabel={c.bcCurrent} />
