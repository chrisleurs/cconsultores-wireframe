import { useState } from "react";
import { Navbar } from "@/components/Navbar";
import { SEO } from "@/components/SEO";
import { FooterC } from "@/components/FooterC";
import { WhatsAppButton } from "@/components/WhatsAppButton";
import { MapPin, Mail, Clock, Loader2, Phone } from "lucide-react";
import { useLang } from "@/i18n/lang";
import { getContactoContent } from "@/i18n/contactoContent";

export default function ContactoC() {
  const lang = useLang();
  const C = getContactoContent(lang);
  const [formState, setFormState] = useState<"idle" | "sending" | "sent">("idle");

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setFormState("sending");
    setTimeout(() => setFormState("sent"), 1500);
  };

  const inputClass =
    "w-full border-0 border-b border-border-subtle rounded-none bg-transparent px-0 py-2.5 font-sans text-base text-camhaji-text placeholder:text-camhaji-muted/40 focus:outline-none focus:border-primary transition-colors";

  return (
    <div className="min-h-screen">
      <SEO
        title={C.seo.title}
        description={C.seo.description}
        path={lang === "en" ? "/en/version-c/contacto" : "/version-c/contacto"}
        lang={lang}
        altPath="/version-c/contacto"
      />
      <Navbar version="c" />

      {/* HERO */}
      <section className="min-h-[50dvh] bg-camhaji-base flex flex-col items-center justify-center text-center px-5 md:px-10 pt-20">
        <p className="label-uppercase text-white/30 mb-6">{C.heroLabel}</p>
        <h1 className="font-sans font-bold text-white mb-5" style={{ fontSize: "clamp(36px, 6vw, 72px)", letterSpacing: "-0.04em", lineHeight: 1.0 }}>
          {C.heroH1}
        </h1>
        <p className="font-sans text-base font-light text-white/50 max-w-[600px] mx-auto" style={{ fontSize: "clamp(16px, 2vw, 20px)", lineHeight: 1.6 }}>
          {C.heroIntro}
        </p>
        <a
          href="https://wa.me/5215543587159"
          target="_blank"
          rel="noopener noreferrer"
          className="btn-uppercase border border-white/50 text-white px-10 py-4 hover:bg-white/10 hover:border-white transition-all duration-300 mt-10"
        >
          {C.heroCta}
        </a>
      </section>

      {/* FORM SECTION */}
      <section className="bg-camhaji-base py-20 px-5 md:px-10">
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-[1fr_400px] gap-16 items-start">
          {/* FORM */}
          <div className="bg-white rounded-sm p-10 md:p-14">
            <h2 className="font-sans font-bold text-camhaji-text mb-2" style={{ fontSize: "clamp(22px, 2.5vw, 30px)", letterSpacing: "-0.02em" }}>
              {C.formTitle}
            </h2>
            <div className="border-b border-border-subtle mb-10 mt-6" />

            <form onSubmit={handleSubmit}>
              <div className="space-y-8 mb-8">
                <div>
                  <label className="label-uppercase text-camhaji-muted mb-2.5 block">{C.labelName}</label>
                  <input type="text" autoComplete="name" placeholder={C.placeholderName} className={inputClass} required />
                </div>
                <div>
                  <label className="label-uppercase text-camhaji-muted mb-2.5 block">{C.labelCompany} <span className="text-camhaji-muted/40">{C.labelOptional}</span></label>
                  <input type="text" autoComplete="organization" placeholder={C.placeholderCompany} className={inputClass} />
                </div>
                <div>
                  <label className="label-uppercase text-camhaji-muted mb-2.5 block">{C.labelService}</label>
                  <select className={`${inputClass} appearance-none cursor-pointer`} required>
                    <option value="">{C.servicePlaceholder}</option>
                    {C.serviceOptions.map((s) => (
                      <option key={s}>{s}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="label-uppercase text-camhaji-muted mb-2.5 block">{C.labelMessage}</label>
                  <textarea rows={3} placeholder={C.placeholderMessage} className={`${inputClass} resize-none`} />
                </div>
              </div>

              <button
                type="submit"
                disabled={formState !== "idle"}
                className={`btn-uppercase w-full px-8 py-4 transition-all duration-300 ${
                  formState === "sent"
                    ? "bg-green-800 text-white"
                    : "bg-primary text-white hover:bg-primary-hover"
                }`}
              >
                {formState === "idle" && C.submitIdle}
                {formState === "sending" && (
                  <span className="flex items-center justify-center gap-2">
                    <Loader2 className="w-4 h-4 animate-spin" /> {C.submitSending}
                  </span>
                )}
                {formState === "sent" && C.submitSent}
              </button>

              <p className="font-sans text-xs text-camhaji-muted mt-4 text-center leading-relaxed">
                {C.submitNote}
              </p>
            </form>
          </div>

          {/* RIGHT PANEL */}
          <div className="md:sticky md:top-[100px] space-y-px">
            <div className="bg-primary rounded-sm p-8">
              <p className="label-uppercase text-white/40 mb-4">{C.rightLabel}</p>
              <div className="flex items-center gap-3 mb-4">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="white">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                <span className="font-sans font-bold text-[22px] text-white" style={{ letterSpacing: "-0.01em" }}>+52 1 55 4358 7159</span>
              </div>
              <p className="font-sans text-[13px] font-light text-white/55 leading-relaxed mb-5">
                {C.rightDesc}
              </p>
              <a
                href="https://wa.me/5215543587159"
                target="_blank"
                rel="noopener noreferrer"
                className="btn-uppercase block text-center border border-white/30 text-white py-3 hover:bg-white/[0.08] hover:border-white/60 transition-all duration-300"
              >
                {C.rightOpen}
              </a>
            </div>

            <div className="bg-white/[0.04] rounded-sm p-7 space-y-4">
              <div className="flex items-start gap-3">
                <MapPin className="w-4 h-4 text-white/35 flex-shrink-0 mt-0.5" />
                <span className="font-sans text-sm text-white/65 leading-relaxed">
                  {C.addressLine}
                </span>
              </div>
              <div className="flex items-start gap-3">
                <svg className="w-4 h-4 text-white/35 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                <a href="https://wa.me/5215543587159" target="_blank" rel="noopener noreferrer" className="font-sans text-sm text-white/65 hover:text-white transition-colors">
                  +52 1 55 4358 7159
                </a>
              </div>
              <div className="flex items-start gap-3">
                <Phone className="w-4 h-4 text-white/35 flex-shrink-0 mt-0.5" />
                <a href="tel:+529988872584" className="font-sans text-sm text-white/65 hover:text-white transition-colors">
                  (998) 887 2584 ext. 1001
                </a>
              </div>
              <div className="flex items-start gap-3">
                <Mail className="w-4 h-4 text-white/35 flex-shrink-0 mt-0.5" />
                <a href="mailto:contacto@camhajiconsultores.mx" className="font-sans text-sm text-white/65 hover:text-white transition-colors">
                  contacto@camhajiconsultores.mx
                </a>
              </div>
              <div className="flex items-start gap-3">
                <Clock className="w-4 h-4 text-white/35 flex-shrink-0 mt-0.5" />
                <span className="font-sans text-sm text-white/65">{C.hours}</span>
              </div>
            </div>
          </div>
        </div>
      </section>

      <FooterC />
      <WhatsAppButton />
    </div>
  );
}